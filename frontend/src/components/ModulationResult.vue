    ...
    <el-row :gutter="16">
      <el-col :span="8">
        <div class="result-card">
          ...
          <div class="value highlight">{{ result.modulation.type }}</div>
        </div>
      </el-col>
      <el-col :span="8">
        <div class="result-card">
      ...
      <el-col :span="8">
        <div class="result-card">
          ...
          <div class="value">{{ symbolRate }} Baud</div>
        </div>
      </el-col>
    </el-row>
    <div class="candidates">
      <div class="label" style="margin-top:12px">候选调制方式</div>
      <div v-for="c in result.modulation.candidates" :key="c.type" class="candidate-row">
...
// 面板只在 ready 时挂载；非空读数统一从这里取
const result = computed(() => store.result!)
// 读数取值规则与原先一致，仅去掉重复的空态兜底
const pct = computed(() => Math.round(result.value.modulation.confidence * 100))
const symbolRate = computed(() => {
  const v = result.value.modulation.symbolRate
